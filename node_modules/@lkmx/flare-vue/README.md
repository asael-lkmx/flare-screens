...
To override those variables, create into ```assets/sass``` the following SCSS files:
...
Then make global ```styles.scss``` by importing it into ```nuxt.config.js```.
...
export default defineNuxtConfig({
    ...
    vite: {
      css: {
        preprocessorOptions: {
          scss: {
            additionalData: '@use "@/assets/sass/styles.scss" as *;'
          }
        }
      }
    }
